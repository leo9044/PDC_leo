...
AmbientControlClient::AmbientControlClient(QObject *parent)
    ...
    , m_isConnected(false)
{
    qDebug() << "[MediaApp] AmbientControlClient created";
}
...
AmbientControlClient::~AmbientControlClient()
{
    ...
    qDebug() << "[MediaApp] AmbientControlClient destroyed";
}

bool AmbientControlClient::initialize()
{
    qDebug() << "🔌 [MediaApp] Connecting to AmbientControl service...";
    ...
    const std::string connection = "client-media-ambient";
...
void AmbientControlClient::setupEventSubscriptions()
{
    ...
    qDebug() << "📡 [MediaApp] Subscribing to AmbientControl events...";
...
void AmbientControlClient::onAmbientColorChanged(std::string newColor)
{
    ...
    qDebug() << "📡 [MediaApp] ambientColorChanged:" << qColor;
...
{
    qDebug() << "📡 [MediaApp] brightnessChanged:" << newBrightness;
...
void AmbientControlClient::fetchCurrentState()
{
    if (!m_proxy || !m_isConnected) {
        qWarning() << "[MediaApp] Cannot fetch state: service not available";
        return;
    }

    qDebug() << "🔍 [MediaApp] Fetching current ambient state...";
...
void AmbientControlClient::onAvailabilityChanged(CommonAPI::AvailabilityStatus status)
{
    ...
    if (m_isConnected != wasConnected) {
        qDebug() << "🔗 [MediaApp] AmbientControl service:"
